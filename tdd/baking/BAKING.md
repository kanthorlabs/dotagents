...
### Step 1 — copy the skeleton into the target project
Copy `tdd/{commands,agents}` and `PROFILE.template.md` into the target repo
(e.g. under `.agents/tdd-src/`). Keep the *source* skeleton + your filled profile
in the repo so the next language port — or a re-bake — re-derives cleanly.
...
the skeleton refuses to guess a language, a command, or a directory. Use
`examples/swift/PROFILE.md` (and the per-language guide here) as a worked model.
...
By hand, by a small substitution script, or by asking the agent to render.
**Marker strings stay literal** (`END:`, `IMPLEMENTATION_READY_FOR_REVIEW:`,
...
- [ ] Every `{{TOKEN}}`/`{{> SLOT}}` is filled; no `{{...}}` remains in the rendered `.claude/` files.
